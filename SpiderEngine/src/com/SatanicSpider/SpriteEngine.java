...
/**
 * @author Nicholas Mamo - Nyphoon Games AND B-Money cuz this shit is fucked
 *
 * TODO: Make this a system and unfuck some shit
 */
public class SpriteEngine
{
	...
	public void addLibrary(SpriteLibrary library)
	{
		...
		library.getNode().setQueueBucket(Bucket.Transparent);
	...
	{
		//TODO: AHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHHH
		//For each library
				...
				if (!e_libraries.get(l).getStatic())
				{
					...
					if (!t_sprite.getPaused() && t_sprite.getAnimated())
					{
						if (t_sprite.getTime() >= t_sprite.getTimeSeparation())
						{
							...
							{
								if (t_sprite.getOnEnd() == Sprite.spriteAction.SCROLL)
								{
							...
							{
								if (t_sprite.getOnEnd() == Sprite.spriteAction.LOOP)
								{
									for (int a = 0; a < t_sprite.getVertexArray().length; a++)
									{
										if (a % 2 == 0)
								...
								{
									if (t_sprite.getOnEnd() == Sprite.spriteAction.NOLOOP)
									{
										for (int a = 0; a < t_sprite.getVertexArray().length; a++)
										{
											if (a % 2 == 0)
									...
									{
										if (t_sprite.getOnEnd() == Sprite.spriteAction.REVERSE)
										{
